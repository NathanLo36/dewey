...
class App(ctk.CTk):
  def __init__(self):
    ...
    # buttons
    self.filter_file_select_button = ctk.CTkButton(
      self.file_info_frame,
      text="Select Filter File",
      command=self.select_filter_file_button_callback,
    )
    ...
    self.open_logs_button = ctk.CTkButton(
      self.control_panel_frame, text="Open logs", command=self.open_logs, height=50
    )
    ...
    log_handler.setFormatter(
      logging.Formatter("%(levelname)s: %(message)s", "%Y-%m-%d %H:%M:%S %z")
    )
